...
class MemoryNode(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "normalized_label", name="uq_memory_nodes_user_label"),
        Index("ix_memory_nodes_user_kind", "user_id", "kind"),
        Index("ix_memory_nodes_user_status", "user_id", "status"),
    )
    ...
    kind: Mapped[str] = mapped_column(String(64), nullable=False, default="concept")
    aliases: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list)
    summary: Mapped[str | None] = mapped_column(Text)
    confidence: Mapped[Decimal | None] = mapped_column(Numeric(4, 3))
    status: Mapped[str] = mapped_column(String(64), nullable=False, default="hypothesis")
...
class MemoryEdge(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_memory_edges_user_status", "user_id", "status"),
    )

    id: Mapped[uuid.UUID] = uuid_pk()
    user_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"))
    source_node_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("memory_nodes.id", ondelete="CASCADE"))
    target_node_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("memory_nodes.id", ondelete="CASCADE"))
...
class MemoryEvidence(Base):
    ...
    __table_args__ = (
        Index("ix_memory_evidence_node", "node_id"),
        Index("ix_memory_evidence_edge", "edge_id"),
        Index("ix_memory_evidence_target", "target_type", "target_id"),
    )

    id: Mapped[uuid.UUID] = uuid_pk()
    user_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"))
    node_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("memory_nodes.id", ondelete="CASCADE"))
    edge_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("memory_edges.id", ondelete="CASCADE"))
    target_type: Mapped[str] = mapped_column(String(64), nullable=False)
    target_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
