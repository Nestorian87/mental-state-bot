...
def verified_semantic_memory_insight(insight: dict, records: list[dict]) -> dict | None:
    if not insight.get("used"):
        return None
    allowed_ids = {str(record.get("target_id") or "") for record in records if record.get("target_id")}
    evidence_ids = [str(value) for value in insight.get("evidence_entry_ids") or [] if str(value) in allowed_ids]
